...
export default {
  ...
  computed: {
    ...
    invalidValues() {
      ...
      if (
        ...
        (!this.isValidValue(this.hourType, this.hour) || this.isDisabled('hour', this.hour))
      ) {
        result.push('hour');
      }
      if (
      ...
      if (
        ...
        (!this.isValidValue(this.apmType, this.apm) || this.isDisabled('apm', this.apm))
      ) {
        result.push('apm');
      }
      if (result.length) {
  ...
  methods: {
    ...
    readStringValues(stringValue) {
      ...
      if (comboReg.test(stringValue)) {
        ...
        if (this.debugMode) {
          ...
          this.debugLog(
            `Successfully parsed values ${JSON.stringify(valueResults)}\nfor ${JSON.stringify(
              tokenChunksForLog,
    ...
    toLeftColumn(currentColumn) {
      const targetItem =
        this.getActiveItemInSideColumn(currentColumn, true) || this.getFirstItemInSideColumn(currentColumn, true);
      if (targetItem) {
        targetItem.focus();
      }
    },
    ...
    toRightColumn(currentColumn) {
      const targetItem =
        this.getActiveItemInSideColumn(currentColumn, false) || this.getFirstItemInSideColumn(currentColumn, false);
      if (targetItem) {
        targetItem.focus();
      }
    },
    ...
    getClosestValidItemInCol(column, currentValue, direction = 'U') {
      if (column === 'hour') {
        ...
        const nextItem =
          direction === 'D' ? this.prevItem(column, this[column], true) : this.nextItem(column, this[column], true);
        if (nextItem) {
